...
function CustomPagination() {
    ...
    return (
        <Pagination className={
            ...
            onChange={
                (event, value) => apiRef.current.setPage(value - 1)
            }/>
    );
}
...
export default function DataTable() {
    ...
    const RefreshPage = () => {
        ...
        axios.get(Constants.url + 'UKP/rest/endpoints/GetAllGenInfoV2').then(function (response) {
            setRows([]);
            setRows(response.data.mGenInfoReportModel);
        }).catch(function (error) {
            if (error.response) {
    ...
    const handleCellDoubleClick = (params, event) => { // alert(event + " $$$ " + params.row.ID);
